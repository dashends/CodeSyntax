...
def get_baseline_topk_scores(correctness, relns, max_k=20, max_offset=19, keywords= []):
    ...
    for reln in relns:
        reln_correctness = np.array(correctness[reln], dtype=bool)
        # print(reln, reln_correctness.shape, max_offset)
        if reln_correctness.shape[0] == 0:
            continue
        topk = np.zeros((reln_correctness.shape[1]), dtype=bool)
        combination=[]
        ...
        for k in range (1, (max_k+1)):
            ...
            for i in range(num_predictors):
                ...
                score = (np.count_nonzero(offset_correctness)/len(offset_correctness)) if len(offset_correctness)!= 0 else 0
            ...
            score = (np.count_nonzero(topk)/len(topk)) if len(topk) != 0 else 0
            reln_scores_topk[reln][k] = (score, combination)
...
add_semicolon_to_keywords = False

for Skip_semicolon, add_semicolon_to_keywords in [(True, False), (False, True)]:
# for Skip_semicolon, add_semicolon_to_keywords in [(False, False), (True, False), (False, True)]:
    output_filename=""
    ...
    dataset_filename="CodeSyntax_java_skip_semicolon.json" if Skip_semicolon else "CodeSyntax_java.json"
    ...
    for metric in ["first", "any", "last"]:
        for partition in ["valid", "test"]:
            ...
            with open("data/attention/cubert_java_full_attn_sorted_"+partition+"_common.pkl", "rb") as f:
            ...
            with open("data/scores/java_full_topk_scores_"+baseline_type+"_"+partition+"_"+metric+output_filename+"_common.pkl", "wb") as f:
